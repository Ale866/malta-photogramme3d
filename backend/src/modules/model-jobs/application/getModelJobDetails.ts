...
import { toModelJobDetailsDto, type ModelJobDetailsDto } from "./jobDetailsDto";
import { badRequest, forbidden, notFound } from "../../../shared/errors/applicationError";

type GetModelJobDetailsInput = {
  jobId: string;
  ownerId: string;
};

export async function getModelJobDetails(services: ModelJobServices, input: GetModelJobDetailsInput): Promise<ModelJobDetailsDto> {
  ...
  return toModelJobDetailsDto(job);
}
